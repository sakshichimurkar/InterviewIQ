...
setAnalyzing(false);
 ...
 console.log(error)
  setAnalysisError(error.response?.data?.message || "Unable to analyze this resume.");
  setAnalyzing(false);
 }
...
const handleStart = async()=>{
  if (!userData) {
    setStartError("Please sign in before starting an interview.");
    return;
  }
...
const result = await axios.post(ServerUrl + "/api/interview/generate-questions", {role, experience, mode, resumeText, projects,skills},
  ...
  onStart(result.data)
    ...
    console.log(error)
    setLoading(false)
    setStartError(error.response?.data?.message || "Unable to start interview. Please try again.")
...
<select value={mode} 
                ...
                {skills.length>0 &&(
                  ...
                  ">
                    ...
                    {skills.map((p,i)=>(
                      ...
                      px-3 py-1 rounded-full text-sm"> {p}</span>
                    ))}
                  </div>
            ...
            hover:bg-green-700 text-white py-3 rounded-lg text-lg font-semibold
